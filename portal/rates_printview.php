...
<html lang="en">
	<head>
		...
		<meta charset="utf-8" />
		<title>Rates Sheet - AwayCare</title>
...
$rates_t_q = mysqli_query($db, "SELECT * FROM `rates_table` WHERE '".$_REQUEST['start']."'>=`eff_date` AND `expiry_date`>='".$_REQUEST['end']."' $addinquery ORDER BY `table_name`");
                                        ...
                                        <thead>
                                            <tr style="background:#ffc464; color:#333;">
                                             ...
                                             <?php 
												...
												$age_q = mysqli_query($db, "SELECT * FROM `ages` WHERE `id`='".$ag_f['age_id']."' ORDER BY `s_age`");
