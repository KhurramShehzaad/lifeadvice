...
border: 5px solid #3a5371 !important;
...
.submit-btn {
font-weight: bold;
background-image: linear-gradient(#007352, #44bc9b);
padding: 10px 35px;
...
.submit-btn:hover {
background-color: #44bc9b;
background-image: linear-gradient(#44bc9b, #007352);
}
.submit-btn i{
	color:#FFF;
}
...
<div class="row plan-details   deductable-<?php echo $deductible; ?>" style="display: <?php if($deductible == '1000'){ echo 'block'; } else if($havethousand == 'no' && $deductible == '0'){ echo 'bock'; } else { echo 'none'; } ?>; padding: 0;">
...
<img style="width:auto; max-height: 110px;" src="admin/uploads/<?php echo $comp_logo; ?>" class="img-responsive" />
...
.no-padding {
	padding:0;
				...
				<div class="hover_<?php echo $deductible.$plan_id;?>"><a href="#" style="display:block; padding:5px;"><i class="fa fa-info-circle"></i> Policy Details</a>
		...
		<?php } ?>	
					...
					<div class="col-md-12 no-padding"><small>Days: <span style="color: #2ba382;"><?php echo $num_of_days;?> (<?php echo $startdate;?> - <?php echo $enddate;?>)</span></small>
					<small>Total: <span style="color: #2ba382;">$<?php echo number_format($total_price,2);?></span></small></div>
					<div class="col-md-12 no-padding"><small>Option: <span style="color: #2ba382;">Deductible Option ($<?php echo $deductible;?> (included in premium))</span></small></div>
...
$person_price = $person_price - $p_discountonplan;
					...
					<div class="col-md-12 no-padding"><small>Insured: <span style="color: #2ba382;"> (Age: <?php echo $person_age; ?>)</span> Coverage Amount: <span style="color: #2ba382;">$<?php echo $sum_insured;?></span> Premium: <span style="color: #2ba382;">$<?php echo number_format($person_price,2); ?></span></small></div>
					<?php } ?>
					</div>
					</div>
					</div>
...
<span style="display:block; padding:5px;">$<?php echo $sum_insured; ?> Benefits Maximum</span>
<?php
$pdfq = mysqli_query($db, "SELECT `pdfpolicy` FROM `wp_dh_insurance_plans_pdfpolicy` WHERE `plan_id`='$plan_id'");
$pdfqry = mysqli_fetch_assoc($pdfq);
$pdfpolicy = $pdfqry['pdfpolicy'];

?>
<a class="btn btn-primary" href="<?php echo $pdfpolicy;?>" target="_blank"><i class="fa fa-file-pdf-o" style="color:#fff;"></i> Sample Policy</a> 
</div>
...
<div class="col-md-12"><strong style="font-size:15px;">Plan Features</strong></div>
		...
		<?php
			$feat = mysqli_query($db, "SELECT `features` FROM `wp_dh_insurance_plans_features` WHERE `plan_id`='$plan_id' ORDER BY `id` ASC");
			while($feature = mysqli_fetch_assoc($feat)){
			$features = explode(':', $feature['features']);
			?>
<div class="row">
			<div class="col-md-4"><strong><?php echo $features[0]; ?></strong></div>
			<div class="col-md-8"><?php echo $features[1];?></div>
</div>
			<?php } ?>			
</div>
...
<p class="text-center">This rate is for $<?php echo $deductible; ?> deductible options</p>
...
?>
<a class="submit-btn" onclick="$('.buynow_<?php echo $deductible.$plan_id;?>').fadeIn();" style="font-weight: bold;padding: 7px 20px;box-shadow: none;border: 1px solid #999;font-size: 16px;display: block;color: #FFF;background: #C00;border-radius: 5px;margin-top: 2px; margin-bottom:10px;"><i class="fa fa-shopping-cart"></i> Buy Now</a>
<div class="compare hidden-xs">
<label style="border: 1px solid #CCC;padding: 10px;text-align: center;display: block;background: #F9F9F9;box-shadow: 0px 0px 1px 0px #CCC inset;border-radius: 3px;cursor: pointer;font-weight: bold;margin-bottom: 10px;"><input data-productid="<?php echo $product_id; ?>" data-pid="<?php echo $plan_id; ?>" price="<?php echo str_replace(',', '', number_format($total_price,2));?>" type="checkbox" tabindex="0" class="hidden1" value="<?php echo str_replace(',', '', number_format($total_price,2));?>" style="height: auto;margin: 0;" onclick="comparetest()">
	...
	<p><input type="checkbox" name="agree" required="" style="height: auto;margin: 0;"> I give permission to LifeAdvice.ca to transfer my quote information and contact details to <?php echo $comp_name;?> in order to complete the purchase of travel insurance. LifeAdvice values your privacy. For details, see our <a href="/">Privacy Policy</a></p>
		<p></p>
	<p><button type="submit" class="submit-btn" style="font-weight: bold;padding: 7px 20px;box-shadow: none;border: 1px solid #999;font-size: 16px;display: block;color: #FFF;background: #C00;border-radius: 5px;margin-top: 2px; margin-bottom:10px;"><i class="fa fa-shopping-cart"></i> Buy Now</button></p>
	</div>
