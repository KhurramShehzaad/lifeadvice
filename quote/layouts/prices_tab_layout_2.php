...
.submit-btn {
font-weight: bold;
background-image: linear-gradient(#007352, #1BBC9B);
padding: 10px 35px;
...
.submit-btn:hover {
background-color: #1BBC9B;
background-image: linear-gradient(#1BBC9B, #007352);
}
.submit-btn i{
	color:#FFF;
}
...
<div class="row filterdiv hidden-xs" style="border: 1px solid #ddd;background: #F9F9F9;text-align: center;padding-top: 10px;border-top: 4px solid #DDD;">	
	...
	<div class="col-md-3 adjust-quoto coverage-mobile-view" style="border-top:0px; ">
			...
			<div class="col-md-3 quote_reference" style="font-size:15px;">
				...
				<span style="color:#C00;"><?php echo $randomquote; ?></span><br/>
...
<div class="row plan-details   deductable-<?php echo $deductible; ?>" style="display: <?php if($deductible == '1000'){ echo 'block'; } else if($havethousand == 'no' && $deductible == '0'){ echo 'bock'; } else { echo 'none'; } ?>; padding: 0; margin-bottom:10px !important;">

<div class="col-md-12" style="border-left: 1px solid #ddd;font-size: 14px;padding: 0;">
<div class="row" style="padding-top: 5px;">


<div class="compare col-md-2 hidden-xs" style="padding-top: 10px;margin: 0;">
<label><input data-productid="<?php echo $product_id; ?>" data-pid="<?php echo $plan_id; ?>" price="<?php echo str_replace(',', '', number_format($total_price,2));?>" type="checkbox" tabindex="0" class="hidden1" value="<?php echo str_replace(',', '', number_format($total_price,2));?>" style="height: auto;margin: 0;" onclick="comparetest()"> Add To Compare</label>
</div>
<div class="col-md-2 col-xs-6 text-center">
<img style="width:100%;" src="admin/uploads/<?php echo $comp_logo; ?>" class="img-responsive"/>
</div>
<div class="col-md-3 col-xs-6 hidden-xs text-center">
<strong>Coverage Amount</strong>
<h2 style="font-size: 23px;font-weight: bold;margin: 0;">$<?php echo $sum_insured; ?></h2>
<h2 style="font-size: 13px;font-weight: bold;margin: 0;">Deductible: $<?php echo $deductible; ?></h2>
</div>

<div class="col-md-2 col-xs-6 text-center">
<strong>Premium</strong>
<h2 style="color: #C00;font-weight: bold;font-size: 26px;font-family: arial;margin: 0;">$<?php echo number_format($total_price,2);?></h2>
...
?>
<a class="submit-btn text-center" onclick="$('.buynow_<?php echo $deductible.$plan_id;?>').fadeIn();" style="font-weight: bold;padding: 7px 10px;box-shadow: none;border: 1px solid #999;font-size: 16px;display: block;color: #FFF;background: #1BBC9B;border-radius: 0;margin-top: 10px;"><i class="fa fa-shopping-cart"></i> Buy Now</a>
</div>
</div>
<div class="row" style="background:#F9F9F9;padding-bottom: 5px;padding-top: 5px;margin: 0;">
<div class="col-md-2 col-xs-6">
<button type="button" class="btn btn-default dh-toggle" data-value='<?php echo $sum_insured.$deductible.$plan_id; ?>' aria-hidden="true" style="width: 100%;display: block;border: 1px solid #BBB;padding: 5px 0;margin-top: 2px;border-radius: 0;background: #F1F1F1 !important;color: #333 !important;height: auto;"><i class="fa fa-plus"></i> More Details</button>
</div>
<div class="col-md-4 col-xs-6 visible-xs">
<a class="submit-btn text-center" onclick="$('.buynow_<?php echo $deductible.$plan_id;?>').fadeIn();" style="font-weight: bold;padding: 5px 10px;box-shadow: none;border: 1px solid #999;font-size: 16px;display: block;color: #FFF;background: #1BBC9B;border-radius: 0;"><i class="fa fa-shopping-cart"></i> Buy Now</a>
</div>
</div>
...
<div class="col-md-12 dh-toggle-show-hide-<?php echo $sum_insured.$deductible.$plan_id; ?>" style="display:none; font-size:16px;">
                                    <div class="col-md-6">
										...
										<strong>Plan Name: </strong> <?php echo $plan_name; ?><br/>
										<strong>Plan Type: </strong> <?php if($family_plan == 'yes'){ echo 'Family'; } else {echo 'Individual';}?><br/>
                                        <strong>Total Premium:</strong> $<?php echo number_format($total_price,2); ?><br/>
										<?php
										$per = 0;
										foreach($ages_array as $person_age){
										$per++;
										?>										
                                        <b>Person <?php echo $per;?></b><br/>
...
$person_price = $person_price - $p_discountonplan;
                                    ...
                                    <div class="col-md-3">
										...
										//
										?>
                                        <b style="color:#C00;"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Features:</b>
                                        <hr/>
                                        <ul style="margin: 0;padding: 0;">
                                            <li style="list-style: none;font-size: 13px;">$<?php echo $deductible; ?> Deductible</li>
											<?php
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <span style="display: none;"><? //"SELECT * FROM wp_dh_insurance_plans_rates WHERE minage <= '$year' AND maxage >= '$year' and sum_insured ='".$_SESSION['sum_insured']."' limit 1000" ?></span>
                                        <hr/>
										<?php
	...
	<p><input type="checkbox" name="agree" required="" style="height: auto;margin: 0;"> I give permission to LifeAdvice.ca to transfer my quote information and contact details to <?php echo $comp_name;?> in order to complete the purchase of travel insurance. LifeAdvice values your privacy. For details, see our <a href="/">Privacy Policy</a></p>
		<p></p>
	<p><button type="submit" class="submit-btn" style="font-weight: bold;padding: 7px 20px;box-shadow: none;border: 1px solid #999;font-size: 16px;display: block;color: #FFF;background: #1BBC9B;border-radius: 0;margin-top: 2px;"><i class="fa fa-shopping-cart"></i> Buy Now</button></p>
	</div>
	...
	<p>or</p>
	<p>BY CALLING</p>
	<p><a href="tel:8555008999" style="font-size:24px; font-weight:bold; color:#1BBC9B;">855-500-8999</a></p>
...
/*
	if (isset( $_SESSION['savers_email'] ) ) {
		...
		include('emailtemplate.php'); //Include (and compiles) the given file
...
//		$headers .= "CC: susan@example.com\r\n";
		$headers .= "BCC: info@lifeadvice.ca \r\n";
		$headers .= "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
		$mailResult = false;
		$mailResult = mail($email, $subject, $message, $headers); //UNCOMMENT THIS
		if($mailResult){
			echo 'email sent';
		} else {
		echo 'email not sent';
